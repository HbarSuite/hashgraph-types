    ...
    export class _Issuer implements IHashgraph.IIssuer {
         ...
         * @constructor
         * @param {IHashgraph.IIssuer} issuer - The issuer object to be converted into an instance of _Issuer
         ...
         * ```
         */
        constructor(issuer: IHashgraph.IIssuer) {
            // Validate DID parameter
            if (!issuer.did || typeof issuer.did !== 'string') {
                throw new Error('Invalid DID');
            }
            this.did = issuer.did;

            // Validate key parameter
            if (!issuer.key || typeof issuer.key !== 'string') {
                throw new Error('Invalid key');
            }
            this.key = issuer.key;

            // Validate fileKey parameter
            if (!issuer.fileKey || typeof issuer.fileKey !== 'string') {
                throw new Error('Invalid file key');
            }
            this.fileKey = issuer.fileKey;

            // Validate optional host parameter if provided
            if (issuer.host !== undefined && typeof issuer.host !== 'string') {
                throw new Error('Invalid host');
            }
            this.host = issuer.host;
        }
    }