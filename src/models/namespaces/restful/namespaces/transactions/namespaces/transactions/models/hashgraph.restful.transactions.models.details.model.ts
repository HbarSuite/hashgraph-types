...
/**
 * @file hashgraph.restful.transactions.models.details.model.ts
 * @class _Details
 * @description Comprehensive model representing detailed transaction information in the Hedera network.
 * Implements IHashgraph.IRestful.ITransactions.ITransaction.IDetails to provide
 * a complete view of a transaction's state and properties, including:
 * - Transaction identification and metadata
 * - Fee structures and processing details
 * - Temporal information and validity windows
 * - Transfer operations (crypto, tokens, NFTs)
 * - Staking rewards and custom fees
 * - Node assignments and scheduling
 * 
 * @implements {IHashgraph.IRestful.ITransactions.ITransaction.IDetails}
 * @category Hashgraph
 ...
 * @example
 * ```typescript
 * // Create a comprehensive transaction details instance
 * const details = new _Details({
 ...
 * ```
 */
export class _Details implements IHashgraph.IRestful.ITransactions.ITransaction.IDetails {
    /**
     ...
     * @type {Array<_CustomFees.AssessedCustomFee>}
     * @optional
     * @remarks
     * - Lists custom fee assessments
     * - Includes fee collectors
     * - Tracks fee amounts
     * - Supports multiple fee types
     */
    @ApiProperty({
        description: 'List of custom fees assessed and collected during transaction processing',
        required: false,
        type: () => [_CustomFees.AssessedCustomFee],
        example: [{
            amount: '1000000',
            collector_account_id: '0.0.123456',
            effective_payer_account_ids: ['0.0.789012'],
            token_id: '0.0.234567'
        }]
    })
    @IsArray()
    ...
    @ValidateNested({ each: true })
    @Type(() => _CustomFees.AssessedCustomFee)
    assessed_custom_fees?: Array<_CustomFees.AssessedCustomFee>;

    /**
     * Creates an instance of _Details
     * @constructor
     * @param {Partial<IHashgraph.IRestful.ITransactions.ITransaction.IDetails>} data - Transaction details data
     * @remarks
     ...
     * @example
     * ```typescript
     * // Create transaction details with transfers
     * const details = new _Details({
     *   transaction_id: "0.0.123456@1234567890.123456789",
     *   transfers: [{
     *     account: "0.0.123456",
     ...
     * ```
     */
    constructor(data: Partial<IHashgraph.IRestful.ITransactions.ITransaction.IDetails>) {
